...
`app-data`，TMDB/简介服务已注册到公共 Koin。探索使用横图和简介兜底，详情使用横图和分集剧照；播放器选集条尚未接剧照。共享
...
| 控制层操作栏 + 上 / 下  | 上键明确回到进度条；下键展开选集条，滚到当前集并送焦                                              |
...
| 操作栏（进度条下） | 后退 10s、下一集、前进 30s、选集、弹幕总开关、选源、倍速、字幕、比例                                                                                                                                                                                                                            |
| 药丸行（进度条上） | 收藏状态在首位，其后推荐、评论、弹幕设置、弹幕列表、画质信息、一起看；已移除角色和制作组药丸                                                                                                                                                                                                                    |
| 收藏        | 下拉更改收藏状态；移除前确认，设为看过后可选择标记全部剧集；有提交忙碌态和失败反馈                                                                                                                                                                                                                         |
| 倍速        | 纵向预设列表聚焦当前值；记住倍速开关、默认速度及长按速度步进；调整范围复用全局配置                                                                                                                                                                                                                         |
| 字幕        | 纵向列出播放器暴露的字幕轨道，可关闭字幕；没有轨道时明确提示                                                                                                                                                                                                                                    |
| 弹幕        | 总开关持久化；字号、透明度、速度、密度、显示区域、描边、字重及顶/底/滚动/彩色类型；来源启停、±500ms 校准/归零、重新搜索条目并匹配剧集                                                                                                                                                                                          |
| 选集        | 底部横条与独立双列网格，初始定位当前集；TMDB 分集剧照优先、缺图回退；长按打开播放/标记看过或未看操作                                                                                                                                                                                                             |
| 选源        | 窗口居中，宽度为扣除水平 48dp 安全边距后可用宽度的 2/3（960dp 屏幕上为 576dp），纵向留出 28dp；顶端分段模式切换，源 tabs 用选中指示条区分层级。简单模式只显示有可用线路的源（无结果或结果全部排除时整组隐藏），按源 icon/名称分组，每组横向 chip row 仅显示线路名，末项右键进入详细模式；详细模式为源 tabrow + 显示排除的源 switch + 详细结果单列，无属性筛选。详细结果显示资源标题和信息标签，不显示包含剧集与链接；支持排除原因、查询状态、重试及验证码入口 |
| 帧预览       | 使用 mediamp FramePreview，192×108dp 画面与时间，2 秒采样网格、70ms 防抖及 12 帧缓存；不支持或提取失败时保留时间反馈                                                                                                                                                                                   |
| 自动跳过      | OP/ED 起点前提示，至少留 5 秒取消窗口；返回键取消本章，本次媒体内不再自动跳该章                                                                                                                                                                                                                      |
| 画质信息      | TV 纵向面板选择可用增强模式，并显示共享 Android 播放器统计                                                                                                                                                                                                                               |
| 一起看       | 加入/创建房间、30 秒超时与取消、房间/成员/连接状态、跟随开关、退出确认；前台状态与共享播放同步接线                                                                                                                                                                                                              |
| 只读内容面板    | 推荐、当前集评论和已加载弹幕保留；按打开的面板订阅数据，推荐点击通过 Intent 导航。与交互面板共用实色底、固定标题和独立滚动列表，面板锚定打开它的药丸并限制在安全区内                                                                                                                                                                            |
...
| 能力                                        | 当前情况                                                    |
|-------------------------------------------|---------------------------------------------------------|
| `TmdbImageService` / `TmdbEpisodeMatcher` | 已在 app-data，包含持久化图片信息与分集匹配；探索横图、详情横图/剧照、播放器选集条/网格剧照均已消费 |
...
| **M1 看番主链路** 🔶 已有实现     | 探索 v5、续播、TMDB 横图/剧照、在线播放、9 个共享扩展；播放器 P0–P2 的预览 seek、双模式选源、倍速/字幕、弹幕开关与属性/匹配、自动跳过、剧集网格/长按、收藏、画质、一起看 | 详情评分/长按/管理动作、完整 DETAILS；自动连播整集、异常换源、不同字幕轨道、房间多人同步及设备矩阵验收                                                        |
...
9. **自定义滚动锚点必须通过 BringIntoView 策略给定**（
   ...
   **；首屏和需要锚定的行应覆盖此行为，避免 hero 被滚掉半屏或行错位。自定义锚点取布局的边，框架原语
   `TvAnchoredBringIntoViewSpec`（ui-foundation-tv/focus：聚焦项对齐容器前缘 + 可动态的预留量）供各页复用：探索页列 = 行头预留 / 继续观看行 = 行首；详情页列仅为
   **hero、简介和选集设置纵向锚点
...
| 播放器选集 / 面板    | 横条卡片 204×114.75dp，间距 12dp；网格双列、间距 16dp、按列宽维持 16:9。内容及设置面板宽 400dp（推荐/一起看 360dp、收藏 248dp）、最高 276dp，距药丸 12dp；设置弹窗宽 480dp（选集 520dp）、最高 460dp；选源宽度为水平安全区（两侧各 48dp）的 2/3，纵向留出 28dp。面板圆角 20dp、行圆角 12dp，聚焦反色、选中淡紫底加对勾 |
