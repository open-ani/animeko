...
| 字符串      | 可直接复用 `stringResource(Lang.xxx)`；播放页已完成共享资源接入，其他 TV 页面的硬编码文案仍需按功能检查                                                         |
...
| 返回    | 弹层逐级关闭回入口；下方区块/简介卡片 → 播放 → 退出；操作栏上下与横排之间显式连接 |
| 编辑 | 收藏与评分使用全屏模糊的页面内覆盖层，按原按钮窗口坐标重绘按钮，操作面板锚定其上方；评分使用共享五颗星、左右加减和分数描述，确认后仅修改分数，保留短评及隐私状态 |

元信息 BGM 评分仅聚焦时显示描边，确认打开模糊背景的评论页，目前只有标题；评论内容与评分分布留待下一阶段。角色、Staff 和关联条目标题统一随行焦点放大/缩小。文件职责、状态、焦点与验证入口见 [TV 条目详情页](docs/contributing/code/tv-subject-details.md)。

详情页所有弹层不使用 `Dialog` / `Popup`。播放页 options 行与面板 API 已抽到 foundation，
两页共用 `TvOptionsRow`、按钮坐标登记和 `TvOptionPanel`，详情页另使用 `TvAnchoredOptionLayout`
与 `TvModalOverlay`，见 [TV options 与覆盖层](docs/contributing/code/tv-options.md)。
...
播放页文案复用 `app-lang` 的英文、简中、港繁、台繁资源。新增文案前，先按同一功能复用共享端的资源与整句映射；仅在共享端没有等价文案时新增 TV 资源，放在各语言目录的 `tv.xml`，不因当前只有 TV 引用就独立定义一套文案。派生中文语言目录由 Gradle 同步所有 XML 资源。一起看的表单、动作、跟随和房主说明复用共享端文案，加入失败也复用完整错误格式；只有退出后果与空成员邀请说明单独维护。剧集序号、弹幕匹配方法、排除原因和错误种类保留为数据，由 UI 在当前语言下解析；焦点身份不依赖译文。验证码类型、弹幕服务名和一起看加入失败原因采用共享 UI 映射，domain 不依赖资源系统。带数量的整句使用位置参数或 plural；用户内容、线路名和字幕轨原始标签保持原值。

上下两行控制按钮按译文宽度、当前字号和可用空间共同决定是否收起文字，图标模式保留无障碍描述。一起看操作允许长文案换行；画质增强仍为横向三段选择，按译文的最小单词宽度分配空间。多语言测试使用进程内 locale 并恢复原环境，导航测试使用稳定 testTag。
...
| 一级侧边栏 + 返回 / 左 | 关闭面板并恢复对应功能 chip 的焦点；不增加可聚焦的面板根节点。一起看加入中同时取消请求 |
| 二级侧边栏 + 返回     | 评论全文回原评论，弹幕列表回设置入口，一起看退出确认回房间；弹幕数值调整结束后留在原参数项 |
| 返回顺序            | 侧边栏逐层返回；未打开侧边栏时优先取消正在提示的自动跳过，其后弹窗/选源 → 面板 → 预览 → 选集条 → 控制层 → 退出 |
...
| 操作栏（进度条下） | 下一集（仅有已播出的下一集时）、选集、弹幕总开关、选源、倍速、字幕、比例。弹幕使用共享端的 Subtitles/SubtitlesOff 图标；选源使用 DisplaySettings，选中资源后展示源 icon 和线路名；宽度不足时隐藏文字 |
| 药丸行（进度条上） | 收藏状态在首位，其后评论、弹幕设置、画质增强、一起看；outlined 样式，不横向滚动，宽度不足时仅显示图标 |
| 中央加载提示 | 复用共享 `EpisodeVideoLoadingIndicator` 及其显示条件，保留自动选源、解析、准备视频、缓冲和具体失败原因；接入播放器的 buffering / media error 状态，字号按 TV 调整 |
...
| 弹幕 | 总开关持久化；参数范围与密度换算复用 `DanmakuConfigRanges`，字号/速度/描边显示相对于共享默认值的百分比，不透明度和显示区域均可设为 0。数值/时间校准项按确定进入调整，左右即时增减，返回结束调整；时间校准仍以 ±500ms 提交，调整中按确定归零。来源启停、重新搜索条目并匹配剧集；设置底部的“弹幕列表”位于“重新匹配弹幕”上方，在侧边栏内进入二级列表，返回恢复入口焦点 |
| 选集 | 直接在药丸上方展开横向剧照条，初始定位当前集；TMDB 分集剧照优先、缺图回退。长按打开播放/标记看过或未看操作，返回恢复原剧集卡片；收起横条后焦点回选集按钮 |
| 选源 | 居中窗口宽度为水平安全区的 2/3（960dp 屏幕上为 576dp），上下留出 28dp。顶部模式按钮聚焦即切换。简单模式按 icon/名称分组，保留查询中、失败、验证码和限流状态；仅隐藏禁用及查询成功但无可用线路的源。每组横向 chip row 显示线路名，末项右键进入详细模式。详细模式为源 tabrow、排除源 switch、详细结果单列。区分重查与处理验证，展示验证中/不支持验证和限流倒计时；仅 `SolveOutcome.Solved` 才清除该源缓存并重查。结果左右键按源层级切换，首源左键回简单模式；没有属性筛选，结果不显示包含剧集或链接 |
| 帧预览 | 位于药丸上方，使用共享 `MediaProgressFramePreviewState` 展示 192×108dp 画面、时间和章节名；2 秒网格、50ms 防抖及 8 帧缓存。关闭 enableFramePreview 或后端不支持时只显示时间/章节；首帧加载显示 circular loading，暂时取不到帧时保留上一帧，切换媒体清空缓存 |
| 自动跳过 | OP/ED 起点前提示，至少留 5 秒取消窗口；返回键取消本章，本次媒体内不再自动跳该章 |
| 画质增强 | 三段式横向按钮选择原始画质、性能优先、画质优先，仅影响当前播放器，不写全局默认模式；下方显示共享 Android 播放器统计 |
| 一起看 | 加入/创建房间、30 秒超时与取消、房间/成员/连接状态、跟随开关、退出确认；复用共享成员及播放状态投影，区分离线/空闲/观看，保留加载和缓冲状态，以服务端时钟计算进度 |
| 只读评论 | 黑色侧边栏中的评论卡片和全文均渲染 BBCode；保留 TV 已确认的正文交互。区分首次加载、空态、首屏失败、追加加载与追加失败，失败可以用遥控器重试。侧边栏无标题栏返回按钮，列表中返回/左键关闭面板并恢复入口焦点；全文返回先回原评论。空态/加载态也可直接关闭，有内容后聚焦评论或重试操作 |
| 推荐条目 | 进度条下任意按钮按下键进入底部推荐横排；底部“显示推荐条目”仅为提示，不获取焦点。控制器和推荐横排共用渐变，以 slide/fade 切换，顶部标题保持；上键/返回回到进度条。标题单行跑马灯，广告项点击 no-op |
...
`TvTogetherCards` 渲染播放卡与平整的双行头像成员列表，`TvTogetherActions` 使用播放器侧栏共用的 `TvOptionRow` 渲染操作与跟随开关，加入进度通过行首 slot 展示。
成员行默认无底色，焦点反色且不缩放；操作和开关统一使用选项行的圆角、左对齐与焦点样式。长成员名和状态只在聚焦时跑马灯展示。
房间名与连接图标、状态文字同处一行，长房间名省略；房主同步说明位于播放卡下方的卡片外，不新增焦点停靠点。跟随整行是唯一的开关焦点，尾部指示器仅显示选中状态。连接状态使用共享枚举，加载期间隐藏无意义的时长进度，
保留共享播放和成员状态文案。房主首次进入聚焦播放卡，访客聚焦跟随开关；只读卡可聚焦浏览但没有点击行为。
成员列表按用户 ID 保留焦点，离开时聚焦相邻成员，清空时回到底部操作，返回内容区前先组合记忆的条目。
退出确认默认聚焦「取消」，取消后回到退出入口；面板根节点不获取焦点，一级页面返回或左键仍直接关闭。
...
| **M1 看番主链路** 🔶 已有实现     | 探索 v5、续播、TMDB 横图/剧照、在线播放、9 个共享扩展；播放器 P0–P2 的预览 seek、双模式选源、倍速/字幕、弹幕开关与属性/匹配、自动跳过、横向选集/长按、收藏、画质增强、一起看 | 详情第二阶段评论与评分分布、完整 DETAILS；自动连播整集、异常换源、不同字幕轨道、房间多人同步及设备矩阵验收                                                        |
...
从播放页迭代归纳的跨页面实践与 review 清单见 [TV 页面开发范式](docs/contributing/tv-development.md)。
新页面先按该文档对照共享端语义、划分状态并定义焦点契约，再结合本章的页面专属裁定实施。

### 14.1 参照与实现方式

1. **功能语义以当前共享端实现和维护者确认的 TV 差异为准**，先找对应功能及状态，再按 Android TV 规范设计视图和遥控器交互。
   PR#3217 的实机效果作为 TV 布局和交互参考，不覆盖后续裁定；需要对照时可安装参考版（包名
   `me.him188.ani.leanback`，与 debug 包 `me.him188.ani.tv.debug2` 并存）。
...
3. **先对照其他端，再做 TV 适配**：实现 UI 前先定位手机/桌面的对应功能、状态来源与恢复流程，记录可用条件、加载/空态/失败、操作结果及配置持久化范围。优先复用状态映射和参数定义，再按 Google Android TV 规范调整布局、字号、焦点和遥控器输入；不得自行改变行为语义。产品明确确认的 TV 差异（例如即时弹幕校准、当前评论正文交互）按已确认方案保留。
...
9. **自定义滚动锚点必须通过 BringIntoView 策略给定**（
   ...
   **hero 设置纵向锚点**（hero 顶边 = 0；简介改为独立全屏阅读，选集由播放页提供）。
   **从角色卡片开始，包含制作人员和关联条目，恢复平台默认纵向滚动，仅保留横向锚定
   **（用户裁定）：这些区块不登记纵向锚点，纵向 spec 未匹配锚点时委托页面覆盖前的默认 spec，不能用自写的最小露出规则代替平台默认；实现见
   `TvDetailsScrollAnchors`/
   `TvDetailsBringIntoViewSpec`。列内横向行 = 行首 + contentPadding，详情页为 58dp。**嵌套的横向滚动容器须独立提供
...
| 详情页   | Google TV 式首屏：全屏背景/线性遮罩、大标题、元信息、半宽简介卡片及三个胶囊操作；角色/Staff 圆形横排，关联条目横版横排。返回由下方区块到播放再退出；播放聚焦回页顶。人物及关联条目使用平台默认纵向滚动，横向行首 + 58dp 独立锚定 |
...
| 详情布局 | 960×540dp 基准：水平 58dp，标题 40sp/54sp，简介卡约 412×116dp/26dp 圆角，胶囊按钮高 40dp，人物卡宽 124dp，关联卡宽 198dp、16:9 |
...
| 文案            | 播放页已接入共享 i18n，含无障碍描述、带参数提示和错误状态；其他 TV 页面仍需资源化                                                                                                                                                            |

**尚未实现的原参考交互
**：列表长按收藏/窥视、列表播放键直接播及长按刷新、播放器完整 DETAILS；弹幕设置和帧预览已按 §8 的 TV 方案实现。原「对话框焦点延迟 300ms」方案已废弃，任何新弹窗都必须遵循 §14.4 的事件驱动焦点规范。
