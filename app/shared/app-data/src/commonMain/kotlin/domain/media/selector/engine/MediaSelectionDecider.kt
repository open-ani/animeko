...
/**
 * 纯函数: 参照用户偏好和各种限制设置, 从 [candidates] 中选择出最合适的 media. 不产生任何副作用.
 *
 * 选择顺序与规则见函数体注释 (#174). 逐字迁自 `DefaultMediaSelector.findUsingPreferenceFromCandidates`,
 * 原先两处挂起读取 (`alliance.available.first()` 与等待 context 加载完成) 提升为参数.
 *
 * @param availableAlliances 当前候选中出现过的全部字幕组 (用于 [MediaPreference.alliancePatterns] 正则匹配).
 * @param mediaSelectorContext 必须已经 [MediaSelectorContext.allFieldsLoaded].
 */
