...
> 2026-09 快速选择重构: ORCH-02~07、FAST-01~06、ERR-05、TIER-03 已按新实现更新(两段超时、精确匹配优先、按 tier 排序)。
...
| ID | 类型 | 测试 | 行为 | 代码 |
|---|---|---|---|---|
| FIND-01 | 设计行为 | ❌ P1 | findUsingPreferenceFromCandidates 开头挂起等待 mediaSelectorContext.allFieldsLoaded()(7 个字段全部非 null)——因此 trySelectDefault/trySelectFromMediaSources 在 context 未加载完成前不会返回结果 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelector.kt`<br>`app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorContext.kt` |
...
| TRY-05 | 设计行为 | ✅ 已覆盖 | trySelectFromMediaSources 基于调用时 filteredCandidates 的一次 first() 快照选择(偏好列表从同一份快照用 filterByPreference 现算, 不再读 preferredCandidates 的 shareIn, 避免两者相差一次更新), 选择过程中 flow 更新不影响本次结果, 函数快速返回; 需要跟随最新数据的调用方自行观察候选流并重调(见 fastSelectWebSources) | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelector.kt` |
| TRY-06 | 已移除 | — | awaitSelectFromMediaSources 已随快速选择重构删除: 其 '候选流每次发射都重跑' 的职责由 fastSelectWebSources 自己的 combine 循环承担(FAST-01/FAST-05)。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelector.kt` |
...
| ORCH-02 | 设计行为 | ✅ 已覆盖 | 自动选择编排是一个 select{} 内 3 个 async clause 的竞速:①上次偏好 Web 源→快速选择(顺序执行) ②选缓存 ③兜底;任一 clause 完成(返回值可为 null)即结束整个 select 并 cancelScope() 取消其余 clause。 _(注:clause 通过 awaitCancellation() 表达'放弃竞争但不结束编排')_ | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelectUseCase.kt` |
| ORCH-03 | 设计行为 | ✅ 已覆盖 | clause① 内部顺序执行:subjectId 可解析时先 trySelectPreferredWebSource, 选中即返回;否则(subjectId 无效或偏好源落空)继续在同一 clause 内启动快速选择——即偏好源路径相对快速选择独占先行,二者不竞速, 快速选择的计时从偏好源落空之后开始。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelectUseCase.kt` |
| ORCH-04 | 设计行为 | ✅ 已覆盖 | 快速选择的门控条件是设置快照 fastSelectWebKind==true 且 preferKind==MediaSourceKind.WEB(settings 在 clause 启动时 .first() 快照),否则该 clause 永久挂起;运行中修改设置不影响已启动的编排。第二段超时取 max(DefaultFuzzyFallbackDuration=15s, fastSelectWebLowTierToleranceDuration)。 _(注:fastSelectWebKind 默认 true;preferKind 新用户默认 WEB,旧用户 null(即旧用户无快速选择))_ | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelectUseCase.kt`<br>`app/shared/app-data/src/commonMain/kotlin/data/models/preference/MediaSelectionSeetings.kt:51-63` |
| ORCH-05 | 设计行为 | ✅ 已覆盖 | clause②(选缓存)与 clause③(兜底)与 clause① 从一开始同时竞速——存在本地缓存时缓存通常先被选中,可赢过上次偏好 Web 源。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelectUseCase.kt` |
| ORCH-06 | 设计行为 | ❌ P0 | clause③ 是唯一可能以 null 结果完成的 clause:awaitCompletedAndSelectDefault 无论是否选中都会返回,从而结束整个编排(包括取消仍在等待中的快速选择);clause①② 结果为 null 时都转为永久挂起。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelectUseCase.kt`<br>`app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelect.kt` |
...
| PREF-04 | workaround | ✅ 已覆盖 | trySelectPreferredWebSource 采用'快照-判空-等待-重试'结构:先立即 trySelect 一次(保持旧时序,避免在上层 select{} 中把机会让给兜底);落空后若 awaitCompletedResults 为空直接返回 null;否则挂起等 filteredCandidates 出现该源的结果后重试一次——这是针对候选流 combine+shareIn(replay=1) 从 cumulativeResults 异步派生导致传播延迟竞态的 workaround。 _(注:MediaFetcher 现已保证 state 变 Succeed 时 results replayCache 完整, 因此 awaitCompletedResults().isEmpty() 的判空可靠;等候选流吸收结果的挂起仍然需要)_ | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelect.kt:205-223` |

## 快速选择

| ID | 类型 | 测试 | 行为 | 代码 |
|---|---|---|---|---|
| FAST-01 | 设计行为 | ✅ 已覆盖 | fastSelectWebSources 只纳入 kind==WEB 的源, 会话无 WEB 源时立即返回 null;它是一个 combine(阶段计时流, 各源 state, filteredCandidates, preferredCandidates) 驱动的响应式循环, 任一输入变化就按当前阶段规则尝试一次 trySelectFromMediaSources(allowNonPreferred=true, 带 blacklist), 选中即返回。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelect.kt` |
| FAST-02 | 设计行为 | ✅ 已覆盖 | 阶段一 INSTANT(开始至 lowTierToleranceDuration):只允许有效 tier(channel tier 优先, 缺失回退源 tier) <= InstantSelectTierThreshold(0) 且 subjectMatchKind==EXACT 的资源;数据源一 Succeed 就选, 不等其他源。模糊匹配的 T0 资源不会被秒选。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelect.kt` |
| FAST-03 | 设计行为 | ✅ 已覆盖 | 阶段二 EXACT_ONLY(至 fuzzyFallbackDuration):放开 tier 限制, 仍只允许 EXACT;对候选中出现过的每个有效 tier 各生成一个过滤器, 按 tier 升序逐档调用 trySelectFromMediaSources, 保证严格 tier 优先级, 不受选择器内部数据源顺序影响。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelect.kt` |
| FAST-04 | 设计行为 | ✅ 已覆盖 | 阶段三 FUZZY(超过 fuzzyFallbackDuration, 或所有 WEB 源都已 final):先 EXACT 按 tier 升序, 再 FUZZY 按 tier 升序。fuzzyFallbackDuration < lowTierToleranceDuration 时视为相等;两者可为 INFINITE(该阶段永不到达)。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelect.kt` |
| FAST-05 | workaround | ✅ 已覆盖 | 一致性检查:候选流从 cumulativeResults 异步派生, 源 state 刚变 Succeed 时候选流可能未吸收其结果。每次快照都校验 '每个 Succeed 源 awaitCompletedResults() 的全部 mediaId 都已出现在 filteredCandidates 中', 不一致的快照不参与选择也不触发终止, 等下一次候选流更新。该检查依赖 MediaFetcher 保证 state 变为 Succeed 时 results 的 replayCache 已含全部结果(见 FETCH 相关改动)。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelect.kt`<br>`app/shared/app-data/src/commonMain/kotlin/domain/media/fetch/MediaFetcher.kt` |
| FAST-06 | 设计行为 | ✅ 已覆盖 | 终止条件:选中→返回 media;overrideUserSelection=false 且 selected 已非 null→返回 null;overrideUserSelection=true 且 selected 与开始时不同(被用户/其他路径改掉)→放弃覆盖返回 null;所有 WEB 源 final 且一致快照下 FUZZY 阶段仍选不出→返回 null;否则一直挂起。override=true 时若当前选择本身满足某一档的过滤器, 到这一档直接保留它(返回它), 不会因 trySelectFromMediaSources '选到同一个' 返回 null 而降级到更差的一档。false 时用 selectDefault CAS 不覆盖用户选择, true(播放失败换源)时 selectImpl(updatePreference=false) 覆盖;两条自动路径均不更新用户偏好、不发 onChangePreference/onPreferWebSource。 | `app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelectorAutoSelect.kt`<br>`app/shared/app-data/src/commonMain/kotlin/domain/media/selector/MediaSelector.kt` |
