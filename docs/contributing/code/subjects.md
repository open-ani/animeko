...
- **主线**（`mainline`）：沿前传/续集关系连接的条目，按故事顺序排列。主线上只有正片（8 话及以上）和剧场版；
  剧场版不计入“第几部”。如果系列有正片，主线的第一个条目一定是正片。
- **相关条目**（`branches`）：列在所属主线条目下的其他条目，包括：
  - 前传/续集链上的特别篇、总集篇、短篇等次要条目，归到它前面最近的正片下（排在第一部正片之前的归到第一部）。
...
图的结构由服务器计算（`GET /subjects/{subjectId}/relation-graph`，最多 100 个条目）。服务器返回前传/续集链上的全部条目
并标出次要条目和总集篇（Bangumi 没有总集篇字段，服务器根据“总集篇”用户标签判断）；哪些条目显示在主线上由客户端的
`SubjectRelationGraphRepository` 决定。
