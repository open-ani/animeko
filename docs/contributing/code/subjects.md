...
- **主线**（`mainline`）：沿前传/续集关系连接的条目，按故事顺序排列。其中正片（8 话及以上）是主要节点，
  剧场版、短篇等是次要节点（`isMinor`），显示为较小的卡片，不计入“第几部”。
- **分支**（`branches`）：番外篇、衍生作品挂在它所属的主线条目下。如果主线本身是另一部作品的衍生
  （例如 `某科学的超电磁炮` 之于 `魔法禁书目录`），原作以“主线故事”关系列在分支中。

从番外篇（例如 OVA）进入关系图时，主线仍然是它所属的正片系列，该番外篇在分支中高亮。

图的结构由服务器计算（`GET /subjects/{subjectId}/relation-graph`，最多 100 个条目），
客户端通过 `SubjectRelationGraphRepository` 获取，并用本地缓存的收藏状态标注每个条目。
UI 位于 `ui/subject/relations`：窄屏为纵向时间线，宽屏为横向时间线。
