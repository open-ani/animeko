...
站点形态各异，而自动选择又要在十几个数据源之间挑出正确的条目和正确的一集，因此过去一份
Selector 配置既要写“怎么把东西列出来”，也要写“怎么判断哪个是对的”：条目名过滤、集号正则、
线路名正则、阶级……编写门槛高，而且站点上名字不含集号的资源（OVA、特典）无论怎么配都选不出来。

现在把配置拆成两层，以“列出来”为基础：

- **列表规则**：搜索结果怎么列、条目页的线路和剧集怎么列、播放页怎么提取视频。只写这一层，
  数据源就已经可以像浏览网站一样使用：搜索、点开条目、选线路、点某一集播放。
- **自动匹配**（`searchConfig.autoMatch`，以及 `tier` / `channelTiers`）：在列表之上让 `fetch`
  自动搜索并筛选出当前剧集的资源。全部可选，缺省时按默认策略自动匹配；`enabled: false`
  表示该源只供浏览，`fetch` 不发起任何请求。

对应到运行时协议，浏览是 `MediaSource` 本身的一部分：按关键字搜索条目、打开条目得到线路与剧集、
把一集转换为 `Media`。它不做任何匹配，关键字由调用方给出，结果原样返回，同名线路也不合并；
`fetch` 是建立在它之上的自动模式。所有数据源都应当是这种形态；尚未迁移的数据源（RSS、BT、媒体库）
暂以“搜索不到任何条目”的默认实现过渡，新数据源必须实现浏览。手动选中一集时由调用方指定它对应哪一集，`Media.episodeRange`
由此而来，这正是自动匹配解决不了的 OVA 场景的出路。同一集不论从哪条路径得到，`mediaId` 相同，
下载去重与偏好记忆因此不区分来源。
...
订阅 JSON 会被所有版本的客户端读取，而导出格式的版本号一旦升高，旧客户端就会整个拒绝该源。
因此自动匹配字段收进 `autoMatch` 时不升版本号，而是双向兼容：读取时没有 `autoMatch`
键就按旧的平铺字段组装，两者都有时以 `autoMatch` 为准；写出时两处都写，旧客户端继续读到平铺字段，
只是不认识 `enabled`，会把只供浏览的源当作普通源使用。平铺字段等旧客户端淘汰后再移除。
